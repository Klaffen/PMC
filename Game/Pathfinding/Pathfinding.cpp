...
std::map<int, int> Pathfinding::getMovementMap(Tile& start, int maxMovement, std::vector<std::vector<Tile>>& tileMap) {
...
std::vector<sf::Vector2i> Pathfinding::getPath(
    Tile& start, Tile& goal, std::map<int, int> movementMap, int maxMovement, std::vector<std::vector<Tile>>& tileMap) {

    if (goal.isTaken || goal.hasUnit || start == goal || movementMap.empty()) {
        return {};
    }
    ...
    if (goalExist) {
        ...
        if ((int) path.size() > maxMovement + 1) {
            return {};
        }
...
std::map<int, int> Pathfinding::neighbourCost(Tile& tile, std::vector<std::vector<Tile>>& tileMap) {
    ...
    if (x > 0) {
        ...
        if (!tileMap[x][y].isTaken && !tileMap[x][y].hasUnit) {
            ...
            neighbours.emplace(tileMap[x][y].id, INF);
        }
    }
...
std::map<int, std::map<int, int>> Pathfinding::getTileGraph(
    Tile& start, int maxMovement, std::vector<std::vector<Tile>>& tileMap) {
    sf::Vector2i startIndex = getTileIndex(start);
    std::map<int, std::map<int, int>> tileGraph; // (example) Tile A, has connections {{B, 1}, {C, 1}, {D, INF}, {E, 1}}
                                                 // where numbers are cost to move there
...
sf::Vector2i Pathfinding::getTileIndex(Tile& position) {
    sf::Vector2i index(
        (int) (position.getPosition().x / Board::TILE_SIZE), (int) (position.getPosition().y / Board::TILE_SIZE));
    return index;
}
