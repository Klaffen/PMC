...
void Shape3D::draw(sf::RenderWindow& window) {
    ...
    transparentRect.setSize(
        sf::Vector2f(top.getSize().x + 2 * outlineThickness, top.getSize().y + 2 * outlineThickness));
    transparentRect.setPosition(
        sf::Vector2f(top.getPosition().x - outlineThickness, top.getPosition().y - outlineThickness));
    ...
    if (top.getPosition().y <= base.getPosition().y
        && top.getPosition().y + top.getSize().y >= base.getPosition().y + base.getSize().y) {
        window.draw(left);
        window.draw(right);
        window.draw(top);
